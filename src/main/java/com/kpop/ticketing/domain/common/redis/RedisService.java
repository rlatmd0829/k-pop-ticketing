...
public class RedisService {
    ...
    public void addToSortedSet(Long value, double score) {
        zSetOperations.add(sortedSetName, value.toString(), score);
    }

    public boolean isExistWaitToken(Long userId) {
        return zSetOperations.score(sortedSetName, userId.toString()) != null;
    }

    // 사용자의 대기번호를 조회하는 메서드
    public Long getWaitNumber(Long userId) {
        Long rank = redisTemplate.opsForZSet().rank(sortedSetName, userId.toString());
